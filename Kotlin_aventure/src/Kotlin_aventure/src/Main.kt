...
fun boirePotion(nomPersonnage : String ,hpPersonnage : Int, nbHpMax : Int,puissancePot : Int ): String {
    var pvInitial = hpPersonnage
    var pvPot = hpPersonnage + puissancePot
   /* pvPot=min(pvPot,nbHpMax)
    pvPot=if(pvPot>nbHpMax) nbHpMax else pvPot*/
    if(pvPot  > nbHpMax){
        pvPot = nbHpMax
    }

       var  recup = pvPot - pvInitial

    return "$nomPersonnage boit une potion et récupère $recup PV"
}
...
fun main() {
    ...
    println(boirePotion("Johan",200,300,150))
